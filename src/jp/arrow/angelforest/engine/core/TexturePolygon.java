...
public class TexturePolygon extends AngelForest2DEngine {
	...
	// テクスチャUV情報
	private final static int[] panelUVs = new int[] { 0, 0, // 左上
			0, one, // 左下
			one, 0, // 右上
			one, one, // 右下
	};
	...
	public TexturePolygon(Context context, Bitmap bitmap) {
		makeTexture(bitmap);
		bitmap.recycle();
	}
	...
	/**
	 * creating square bitmap canvas and draw image on theere. (it seems that
	 * png image needs to be rewrite to bitmap in order to display it...)
	 * 
	 * @param bmp
	 * @param w
	 * @param h
	...
	/**
	 * Creating vertices for the polygon. initial polygon will be drawn as a
	 * square and is placed at the center (0, 0).
	 * 
	 * @param texture
	 * @return
	 */
	private int[] resizeMatrix(Texture texture) {
		final int[] panelVertices = new int[] { -one * texture.size / 2,
				one * texture.size / 2, 0,// 左上
